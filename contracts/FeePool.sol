...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    function closeSecondary(uint _allNetworksSnxBackedDebt, uint _allNetworksDebtSharesSupply) external onlyRelayer {
        _closeSecondary(_allNetworksSnxBackedDebt, _allNetworksDebtSharesSupply);
    }

    /**
     * @notice Close the current fee period and start a new one.
     */
    function _closeSecondary(uint _allNetworksSnxBackedDebt, uint _allNetworksDebtSharesSupply) internal {
        ...
        _recentFeePeriodsStorage(0).allNetworksSnxBackedDebt = _allNetworksSnxBackedDebt;
